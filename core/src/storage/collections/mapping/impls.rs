...
impl<K, V> Mapping<K, V> {
    ...
    pub fn len(&self) -> u32 {
        *self.len.get().expect(
            "[liquid_core::Mapping::len] Error: expected `len` field to be existed in \
             storage",
        )
    }
    ...
    Q: Encode,
{
    ...
    fn index(&self, index: &'a Q) -> &Self::Output {
        self.get(index)
            .expect("[liquid_core::Mapping::index] Error: expected `index` to be existed")
    }
}
    ...
    Q: Encode,
{
    fn index_mut(&mut self, index: &'a Q) -> &mut Self::Output {
        self.get_mut(index).expect(
            "[liquid_core::Mapping::index_mut] Error: expected `index` to be existed",
        )
    }
}
